...
const CUISINE_POTENTIAL_RISK=["sushi","sashimi","izakaya","raw","oyster","yakitori","yakiniku","bbq","tartar","ceviche","fondue","blue_cheese"];
...
function friendlyCuisine(tags){const t=typeFromTags(tags);if(t==="bakery")return"ベーカリー/パン";if(tags.amenity==="cafe"||tags.amenity==="ice_cream")return"カフェ/甘味";const c=cuisineList(tags);if(c.includes("udon")||c.includes("soba"))return"麺類（うどん/そば）";if(c.includes("ramen"))return"ラーメン";if(c.includes("japanese")||c.includes("teishoku"))return"和食/定食";if(c.includes("family"))return"ファミレス";if(c.includes("sandwich")||c.includes("bakery"))return"軽食・パン";if(c.includes("indian")||c.includes("thai"))return"エスニック";if(c.includes("italian"))return"イタリアン";if(c.length)return c[0];return"飲食店";}
...
async function fetchMeals(lat,lon,minutes,profile,catSet,prefSet,avoidSet,excludeWords,limit=10){const kmh=speedToKmh(profile);const r=Math.max(1,(kmh*(minutes/60))*0.6);const q=buildQuery(lat,lon,r);const res=await fetch("https://overpass-api.de/api/interpreter",{method:"POST",body:q,headers:{"Content-Type":"text/plain"}});const json=await res.json();const elements=json.elements||[];const items=elements.map(e=>{const latc=e.lat||(e.center&&e.center.lat);const lonc=e.lon||(e.center&&e.center.lon);const tags=e.tags||{};const name=tags.name||tags["name:ja"]||"お店";return{id:e.id,lat:latc,lon:lonc,tags,name};}).filter(x=>x.lat&&x.lon);
  const results=items.map(s=>{const nameLower=(s.name||"").toLowerCase();if(excludeWords.some(w=>w&& (nameLower.includes(w)||(s.tags["name:ja"]||"").includes(w))))return null;const cuisines=cuisineList(s.tags);const t=typeFromTags(s.tags);const flags=riskFlags(s.tags);
    let catScore=0;if(catSet.has("japanese")&&(cuisines.includes("japanese")||cuisines.includes("teishoku")))catScore-=2;if(catSet.has("noodles")&&(cuisines.includes("udon")||cuisines.includes("soba")||cuisines.includes("ramen")))catScore-=2;if(catSet.has("cafe")&&(t==="cafe"||t==="ice_cream"))catScore-=2;if(catSet.has("bakery")&&(t==="bakery"||cuisines.includes("bakery")||cuisines.includes("sandwich")))catScore-=2;if(catSet.has("family")&&(cuisines.includes("family")))catScore-=2;
    let prefScore=0;if(prefSet.has("low_odor"))prefScore+=odorScore(s.tags);if(prefSet.has("mild_spicy"))prefScore+=spicyScore(s.tags);
    let riskScore=0;for(const k of avoidSet){if(flags[k])riskScore+=4;}
    ...
    const total=catScore+prefScore+riskScore+distScore+(status.open===true?-0.5:0); // 営業中は少し優遇
    return {...s,distKm,catScore,prefScore,riskScore,total,flags,ohParsed:oh,openStatus:status};
  }).filter(Boolean).sort((a,b)=>a.total-b.total).slice(0,limit);
  return results;
}
function renderList(items){const ul=document.getElementById("list");ul.innerHTML="";if(!items.length){const li=document.createElement("li");li.className="item";li.textContent="条件に合うお店が見つかりませんでした。条件を緩めて再検索してください。";ul.appendChild(li);return;}
  items.forEach(s=>{const tags=[];const f=s.flags;if(f.raw_fish)tags.push("生魚注意");if(f.raw_egg)tags.push("生卵注意");if(f.alcohol)tags.push("アルコール中心");if(f.soft_cheese)tags.push("チーズ注意");if(f.high_mercury)tags.push("水銀多注意");if(f.deli_meat)tags.push("デリミート注意");
    ...
    li.innerHTML=`<div><strong>${s.name}</strong> <span class="badge">${friendlyCuisine(s.tags)}</span> ${badge}${hours}
      <div class="meta">約${s.distKm.toFixed(1)}km／${explainReason(s.tags,s.flags)}</div>
      <div class="tags">${tags.map(t=>`<span class="tag">${t}</span>`).join("")}</div>
    </div>
    <div class="actions">
      <a class="btn small" target="_blank" rel="noopener" href="${gmapsLink(s.lat,s.lon)}">Googleマップ</a>
      <a class="btn small" target="_blank" rel="noopener" href="${appleMapsLink(s.lat,s.lon,s.name)}">Appleマップ</a>
    </div>`;
    ul.appendChild(li);
  });}
function getSets(){const cats=new Set([...document.querySelectorAll(".cat:checked")].map(x=>x.value));const prefs=new Set([...document.querySelectorAll(".pref:checked")].map(x=>x.value));const avoids=new Set([...document.querySelectorAll(".avoid:checked")].map(x=>x.value));const exclude=document.getElementById("exclude").value.split(",").map(s=>s.trim().toLowerCase()).filter(Boolean);return{cats,prefs,avoids,exclude};}
async function run(){const region=document.getElementById("region");const minutes=parseInt(document.getElementById("driveMin").value,10)||15;const profile=document.getElementById("speedProfile").value;let lat,lon;if(region.value==="current"){try{const pos=await new Promise((res,rej)=>navigator.geolocation.getCurrentPosition(res,rej,{enableHighAccuracy:true,timeout:8000}));lat=pos.coords.latitude;lon=pos.coords.longitude;}catch(e){alert("現在地が取得できませんでした。江坂を使用します。");region.value="esaka";lat=presets.esaka.lat;lon=presets.esaka.lon;}}else{lat=presets[region.value].lat;lon=presets[region.value].lon;}
  const {cats,prefs,avoids,exclude}=getSets();try{const items=await fetchMeals(lat,lon,minutes,profile,cats,prefs,avoids,exclude,10);renderList(items);}catch(e){document.getElementById("list").innerHTML="<li class='item'>Overpassの取得に失敗しました。時間をおいて再試行してください。</li>";}}
