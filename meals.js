...
function openGoogleMapsApp(name, lat, lon){
  ...
  const appUrl = `comgooglemaps://?q=${q}&center=${lat},${lon}`;
  const webUrl = `https://www.google.com/maps/search/?api=1&query=${q}&center=${lat}%2C${lon}`;
  const isIOS = /iPad|iPhone|iPod/.test(navigator.userAgent);
  if(isIOS){
    const t = Date.now();
    window.location.href = appUrl;
    setTimeout(()=>{ if (Date.now() - t < 1500) window.location.href = webUrl; }, 800);
  }else{
...
function riskFlags(tags){const name=(tags.name||tags['name:ja']||'').toLowerCase();const c=cuisineList(tags);return{raw_fish:c.includes('sushi')||c.includes('sashimi')||name.includes('寿司')||name.includes('刺身'),raw_egg:name.includes('生卵')||name.includes('すき焼き')||c.includes('sukiyaki'),alcohol:c.includes('bar')||c.includes('pub')||c.includes('izakaya')||name.includes('居酒屋'),soft_cheese:c.includes('cheese')||c.includes('pizza')||c.includes('italian'),high_mercury:c.includes('tuna')||name.includes('マグロ'),deli_meat:name.includes('生ハム')||name.includes('ローストビーフ')||name.includes('低温調理')};}
function friendlyCuisine(tags){const t=typeFromTags(tags);if(t==='bakery')return'ベーカリー/パン';if(tags.amenity==='cafe'||tags.amenity==='ice_cream')return'カフェ/甘味';const c=cuisineList(tags);if(c.includes('udon')||c.includes('soba'))return'麺類（うどん/そば）';if(c.includes('ramen'))return'ラーメン';if(c.includes('japanese')||c.includes('teishoku'))return'和食/定食';if(c.includes('family'))return'ファミレス';if(c.includes('sandwich')||c.includes('bakery'))return'軽食・パン';if(c.includes('indian')||c.includes('thai'))return'エスニック';return'飲食';}
...
function gmapsOpenHandler(name,lat,lon){openGoogleMapsApp(name,lat,lon);}
...
async function fetchMeals(lat,lon,minutes,profile,catSet,prefSet,avoidSet,excludeWords,limit=10){
  ...
  const results = items.map(s=>{
    ...
    if(catSet.has("bakery") && (t==="bakery") ) catScore-=2;
    ...
    const oh=(s.tags&&s.tags.opening_hours)||''; const status=(function(ohStr){const p=parseOpeningHours(ohStr);return isOpenNow(p,new Date());})(oh);
...
function renderList(items){
  ...
  items.forEach(s=>{
    ...
    <div class="actions vstack">
      <button class="iconbtn gmaps" title="Googleマップで開く"><svg viewBox='0 0 24 24' aria-hidden='true'><path d='M12 2C7.6 2 4 5.6 4 10c0 6 8 12 8 12s8-6 8-12c0-4.4-3.6-8-8-8z' fill='#EA4335'/><circle cx='12' cy='10' r='5' fill='#fff'/><text x='12' y='13' text-anchor='middle' font-size='8' font-family='Arial' fill='#1a73e8' font-weight='700'>G</text></svg></button>
      <a class="iconbtn amaps" target="_blank" rel="noopener" title="Appleマップで開く" href="#"><svg viewBox='0 0 24 24' aria-hidden='true'><rect x='3' y='3' width='18' height='18' rx='4' fill='#e2e8f0' stroke='#cbd5e1'/><path d='M7 14l3-3 2 2 4-4' fill='none' stroke='#0ea5e9' stroke-width='2'/><circle cx='16' cy='9' r='2' fill='#94a3b8'/></svg></a>
    </div>`;
    ul.appendChild(li);
    li.querySelector('.gmaps').addEventListener('click',()=>openGoogleMapsApp(s.name,s.lat,s.lon));
    li.querySelector('.amaps').setAttribute('href', appleMapsHref(s.name,s.lat,s.lon));
  });
}
