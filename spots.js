...
function appleMapsLink(s){const q=encodeURIComponent(s.tags.name||s.tags['name:ja']||'目的地');return `https://maps.apple.com/?ll=${s.lat},${s.lon}&q=${q}`;}
...
async function search(lat,lon,minutes,profile,limit=10){const kmh=speedToKmh(profile);const r=Math.max(1,(kmh*(minutes/60))*0.6);const q=buildOverpassQuery(lat,lon,r);const res=await fetch("https://overpass-api.de/api/interpreter",{method:"POST",body:q,headers:{"Content-Type":"text/plain"}});const json=await res.json();const es=(json.elements||[]).map(e=>{const latc=e.lat||(e.center&&e.center.lat);const lonc=e.lon||(e.center&&e.center.lon);const tags=e.tags||{};const name=tags.name||tags['name:ja']||tags['name:en']||'スポット';return {id:e.id,lat:latc,lon:lonc,tags,name};}).filter(e=>e.lat&&e.lon);
  const items=es.map(s=>{const props=mapProps(s.tags);const distKm=haversine(lat,lon,s.lat,s.lon);return {...s,props,distKm};}).sort((a,b)=>a.distKm-b.distKm).slice(0,10);
  return items;
}
function render(items){const ul=document.getElementById('spots');ul.innerHTML='';if(!items.length){const li=document.createElement('li');li.className='spot';li.textContent='見つかりませんでした。';ul.appendChild(li);return;}items.forEach(s=>{const li=document.createElement('li');li.className='spot';li.innerHTML=`<div><strong>${s.name}</strong><div class='meta'>${shortDesc(s)}</div></div><div class='actions vstack'><button class='iconbtn gmaps' title='Googleマップで開く'>${GOOGLE_SVG}</button><a class='iconbtn amaps' target='_blank' rel='noopener' title='Appleマップで開く' href='${appleMapsLink(s)}'>${APPLE_SVG}</a></div>`;ul.appendChild(li);li.querySelector('.gmaps').addEventListener('click',()=>openGoogleMapsApp(s.name,s.lat,s.lon));});}
async function run(){let lat=34.7565,lon=135.4968;try{const items=await search(lat,lon,20,'normal',10);render(items);}catch(e){document.getElementById('spots').innerHTML="<li class='spot'>取得に失敗しました。</li>";}}
